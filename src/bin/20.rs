...
pub fn part_one_with_limit(input: &str, limit: usize) -> Option<usize> {
    ...
    for y in 0..max_y {
        for x in 0..max_x {
            ...
            maze.maze[y][x] = b'.';
            let new_solve =
                get_steps_for_maze(&maze).expect("removing a wall made the maze unsolvable?");
            ...
            if let std::collections::hash_map::Entry::Vacant(e) =
                better_routes.entry(route_improvement)
            {
                e.insert(vec![(x, y)]);
            } else {
                better_routes
                    .get_mut(&route_improvement)
                    .unwrap()
                    .push((x, y));
            }
            maze.maze[y][x] = b'#';
    ...
    for key in better_routes
        .keys()
        .sorted()
        .filter(|k| **k >= min_better_amount)
    {
        better_count += better_routes[key].len();
        println!(
            "Found {} routes that are {} better",
            better_routes[key].len(),
            key
        );
    }
    Some(better_count)
...
pub fn part_two_ex(input: &str, shortcut_distance: i32, min_improvement: usize) -> Option<u32> {
    ...
    for here in route.iter() {
        get_list_of_nearby_spots(*here, shortcut_distance, &maze)
            .iter()
            .for_each(|shortcut| {
                ...
                if steps_to_shortcut > (steps_to_here + steps_from_here_to_shortcut) {
                    ...
                    let steps_to_end_from_here =
                        maze_steps_for_each_position[&maze.end] - steps_to_here;
                    let steps_to_end_from_shortcut =
                        maze_steps_for_each_position[&maze.end] - steps_to_shortcut;
                    let steps_to_end_via_shortcut =
                        steps_from_here_to_shortcut + steps_to_end_from_shortcut;
    ...
    for key in shortcut_count.keys().sorted() {
        if key >= &min_improvement {
            ...
            println!(
                "Found {} shortcuts that save {} steps",
                shortcut_count[key], key
            );
        } else {
            println!(
                "Found {} shortcuts that save {} too few steps",
                shortcut_count[key], key
            );
        }
    }
