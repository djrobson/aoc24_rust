...
fn search_operators(total: usize, operands: Vec<usize>) -> bool {
    if operands.len() == 1 {
    ...
    search_operators(total, add_operands) || search_operators(total, mul_operands)
}
...
pub fn part_one(input: &str) -> Option<usize> {
    ...
    for (cal_total, operands) in calibrations {
        if search_operators(cal_total, operands) {
            total += cal_total;
        }
    }
    Some(total)
